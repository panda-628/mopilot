...
if running_params['llm'] == 'gpt3.5':
    new_folder = 'tem-' + str(running_params['temperature_relation']) + 'time-' + str(time.time())
...
for name,description,oracle_classes,oracle_relationships in zip(name_list,description_list,class_name_list_str_list,oracle_relationships_list):
    ...
    f_relation = open(output_relation_file,'w')
    ...
    for c in range(1,cycle+1):
        ...
        print(f'---------------------{c}/{cycle}------{name}:',file=f_relation)
        ...
        print(f'AI_answer_after_cut:{AI_answer}',file = f_relation)
        ...
        if mat.generated_associations_count == 0:
            presicion = 0
        else:
            presicion = mat.matched_associations_count / mat.generated_associations_count
            total_presicion += presicion
        recall = mat.matched_associations_count / mat.oracle_associations_count
        total_recall += recall
        if presicion + recall == 0:
            ...
            total_F1 += F1
        print(f'presicion = {presicion}',file=f_relation)
        print(f'recall = {recall}',file=f_relation)
        print(f'F1 = {F1}',file=f_relation)
    average_presicion = total_presicion / cycle
    average_recall = total_recall / cycle
    average_F1 = total_F1 / cycle
    print(f'average_presicion = {average_presicion}',file=f_relation)
    print(f'average_recall = {average_recall}',file=f_relation)
    print(f'average_F1 = {average_F1}',file=f_relation)
